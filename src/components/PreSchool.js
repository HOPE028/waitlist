...
export default function PreSchool() {
  ...
  const waitListCollectionRef = collection(db, 'PreSchool')
  const statsCollectionRef = doc(db, 'Stats', 'preSchoolAgeID')
  ...
  const handleDelete = async (child) => {
    if (window.confirm(`Delete ${child.name}'s account?`)) {
      const childRef = doc(db, 'PreSchool', child.id)
...
function EditChild(props) {
  ...
  const handleClick = async () => {
    ...
    try {
      const childRef = doc(db, 'PreSchool', props.child.id)
...
function NewChild(props) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const PreSchoolCollectionRef = doc(db, 'PreSchool', document.id)

      const currentOrder = Number(props.order) + 1

      await setDoc(PreSchoolCollectionRef, {
        name: childName,
