...
export default function SchoolAge() {
  ...
  const waitListCollectionRef = collection(db, 'SchoolAge')
  const statsCollectionRef = doc(db, 'Stats', 'schoolAgeID')
  ...
  const handleDelete = async (child) => {
    if (window.confirm(`Delete ${child.name}'s account?`)) {
      const childRef = doc(db, 'SchoolAge', child.id)
      ...
      CorrectOrder('SchoolAge', waitList, child.order)
...
function EditChild(props) {
  ...
  const handleClick = async () => {
    ...
    try {
      const childRef = doc(db, 'SchoolAge', props.child.id)
...
function NewChild(props) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const document = await addDoc(kidsCollectionRef, {
        ageGroup: 'SchoolAge',
      })

      const SchoolAgeCollectionRef = doc(db, 'SchoolAge', document.id)

      const currentOrder = Number(props.order) + 1

      await setDoc(SchoolAgeCollectionRef, {
        name: childName,
